...
async function writeStore(store) {
  ...
  const orderOps = orders.map((order) => ({
    replaceOne: { filter: { id: order.id }, replacement: order, upsert: true },
  }));
  ...
  await Promise.all([
    userOps.length ? mongoCollections.users.bulkWrite(userOps) : Promise.resolve(),
    productOps.length ? mongoCollections.products.bulkWrite(productOps) : Promise.resolve(),
    orderOps.length ? mongoCollections.orders.bulkWrite(orderOps) : Promise.resolve(),
    mongoCollections.users.deleteMany({ id: { $nin: userIds } }),
    mongoCollections.products.deleteMany({ id: { $nin: productIds } }),
    mongoCollections.orders.deleteMany({ id: { $nin: orderIds } }),
    mongoCollections.settings.replaceOne(
      { _id: MONGO_SETTINGS_ID },
...
function parseBody(req, maxLimit = 15e6) {
...
async function sendOtpViaBrevo(email, otp) {
  ...
  if (response.statusCode < 200 || response.statusCode >= 300) {
    ...
    try {
      ...
      reason = parsed.message || parsed.code || "";
    } catch (_) {
      reason = "";
    }
    throw new Error(`Email OTP send failed (${response.statusCode})${reason ? `: ${reason}` : ""}`);
  }
}
...
async function handleApi(req, res, parsedUrl) {
  ...
  if (req.method === "POST" && pathname === "/api/admin/upload") {
    ...
    const allowed = ["jpg", "png", "webp", "gif", "svg"];
    ...
    const filename = `saree_${Date.now()}_${randomSuffix}.${ext}`;
    const targetPath = path.join(uploadsDir, filename);

    try {
      fs.writeFileSync(targetPath, Buffer.from(matches[2], "base64"));
      sendJson(res, 201, { url: `/uploads/${filename}`, filename });
    } catch (err) {
      sendJson(res, 500, { error: "Failed to save file", details: err.message });
    }
    return;
  }
