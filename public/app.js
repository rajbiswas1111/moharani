...
async function uploadFileThroughApi(file) {
            ...
            (order) => `
              <article class="profile-order-card">
                ...
                <div class="inline-actions" style="margin-top:8px;">
                  ...
                  <a class="small-btn" href="./invoice.html?id=${order.id}" target="_blank">📄 Invoice</a>
                </div>
              </article>
            `
          )
...
async function renderOrderTrackingPage() {
  ...
  mount.innerHTML = `
    ...
    <div style="margin-top: 20px; display: flex; gap: 10px;">
      <a class="btn btn-primary" href="./invoice.html?id=${order.id}" target="_blank" style="padding: 10px 20px; font-size: 0.88rem;">📄 Download / Print Tax Invoice</a>
    </div>
  `;
}
...
async function renderProductDetail() {
  ...
  mount.innerHTML = `
    <section class="detail-grid">
      ...
      <div class="panel detail-copy">
        ...
        <div class="delivery-tag detail-delivery-tag">Free Delivery Available</div>

        <div class="pincode-estimator">
          <label style="font-size:0.8rem; font-weight:700; color:#701625;">📍 Check Estimated Delivery & COD</label>
          <div class="pincode-input-row">
            <input type="text" id="pincodeInput" placeholder="Enter 6-digit PIN" maxlength="6" />
            <button type="button" id="checkPinBtn">Check</button>
          </div>
          <div id="pincodeResult" class="pincode-result"></div>
        </div>

        <table class="saree-specs-table">
          <tbody>
            <tr><td>Length</td><td>5.5 Meters Saree + 0.8 Meter Blouse Piece</td></tr>
            <tr><td>Work / Weave</td><td>Handcrafted Artisanal Weaving</td></tr>
            <tr><td>Occasion</td><td>${product.category || "Celebration"}</td></tr>
            <tr><td>Care Guide</td><td>Dry Clean Recommended</td></tr>
          </tbody>
        </table>
        ...
        <div class="inline-actions" style="margin-top: 14px;">
          ...
          <a class="btn-whatsapp-inquiry" href="https://wa.me/918945998947?text=${encodeURIComponent("Hello The Moharani, I am interested in: " + product.name + " (" + currency(product.price) + "). Is this available?")}" target="_blank">
            💬 WhatsApp Inquiry
          </a>
        </div>
      </div>
  ...
  const pinBtn = mount.querySelector("#checkPinBtn");
  const pinInput = mount.querySelector("#pincodeInput");
  const pinResult = mount.querySelector("#pincodeResult");
  if (pinBtn && pinInput && pinResult) {
    pinBtn.addEventListener("click", () => {
      const pin = pinInput.value.trim();
      if (!/^\d{6}$/.test(pin)) {
        pinResult.innerHTML = "<span style='color:#c0392b;'>Please enter a valid 6-digit Indian PIN code.</span>";
        return;
      }
      const deliveryDate = new Date(Date.now() + 4 * 86400000).toLocaleDateString("en-IN", { month: "short", day: "numeric" });
      pinResult.innerHTML = `<span style='color:#1f6a45;'>✓ Delivery available to ${pin} by <strong>${deliveryDate}</strong>. Cash on Delivery supported.</span>`;
    });
  }
...
function renderCheckoutSummary(selectedMethod = "Cash on Delivery") {
  ...
  const isOnlinePay = selectedMethod === "Online Pay (WhatsApp)";
  const discount = isOnlinePay ? Math.round(total * 0.1) : 0;
  ...
  mount.innerHTML = `
    ...
    ${
      discount
        ? `<div class="summary-line"><span>Online Pay Discount (10%)</span><strong>- ${currency(discount)}</strong></div>`
        : ""
    }
    <div class="summary-line"><span>Total Payable</span><strong>${currency(finalTotal)}</strong></div>
  `;
}
...
function showOrderSuccessAndRedirect(orderId) {
  ...
  const dialog = modal.querySelector(".success-dialog");
  if (dialog) {
    const desc = dialog.querySelector("p");
    if (desc && orderId) {
      desc.innerHTML = `Your order <strong>${orderId}</strong> has been placed successfully!`;
    }
    let actionBox = dialog.querySelector(".modal-order-actions");
    if (!actionBox) {
      actionBox = document.createElement("div");
      actionBox.className = "modal-order-actions";
      actionBox.style.cssText = "margin-top: 22px; display: flex; gap: 10px; justify-content: center; flex-wrap: wrap;";
      dialog.appendChild(actionBox);
    }
    actionBox.innerHTML = `
      ${orderId ? `<a class="btn btn-primary" href="./invoice.html?id=${orderId}" target="_blank" style="padding: 10px 18px; font-size:0.88rem;">📄 Print Tax Invoice</a>` : ""}
      ${orderId ? `<a class="small-btn" href="./order-tracking.html?id=${orderId}">Track Order</a>` : ""}
      <a class="small-btn" href="./products.html">Continue Shopping</a>
    `;
  }
...
async function startRazorpayCheckout(payload, form, message) {
  ...
  const verifiedOrder = await new Promise((resolve, reject) => {
    const options = {
      key: orderData.keyId,
      ...
      handler: async (response) => {
        try {
          const verifyData = await api("/api/payment/razorpay/verify", {
            method: "POST",
            body: JSON.stringify({
              customer: payload,
              items: checkoutItems,
  ...
  showOrderSuccessAndRedirect(verifiedOrder?.order?.id || "");
}

function openWhatsappPayment(payload, checkoutItems, totals, orderId) {
  const phone = "918945998947";
  const itemSummary = checkoutItems
    .map((item) => `${item.productId} x${item.quantity}`)
    .join(", ");
  const discount = Math.round((totals.total || 0) * 0.1);
  const finalTotal = (totals.total || 0) - discount;
  const text =
    `Hello The Moharani,%0A` +
    `I want to pay online for my order.%0A` +
    `Order ID: ${encodeURIComponent(orderId || "")}%0A` +
    `Name: ${encodeURIComponent(payload.name || "")}%0A` +
    `Phone: ${encodeURIComponent(payload.phone || "")}%0A` +
    `Items: ${encodeURIComponent(itemSummary)}%0A` +
    `Total: ${encodeURIComponent(currency(totals.total))}%0A` +
    `Discount (10%): ${encodeURIComponent(`- ${currency(discount)}`)}%0A` +
    `Payable: ${encodeURIComponent(currency(finalTotal))}`;
...
function setupCheckout() {
  ...
  const discountHeading = document.getElementById("onlineDiscountHeading");
  const methodField = form.elements.method;
  const updateCheckoutView = () => {
    const method = methodField?.value || "Cash on Delivery";
    renderCheckoutSummary(method);
    if (discountHeading) {
      discountHeading.classList.toggle("hidden", method !== "Online Pay (WhatsApp)");
    }
  };
  updateCheckoutView();
  methodField?.addEventListener("change", updateCheckoutView);
  if (state.auth?.user) {
  ...
  form.addEventListener("submit", async (event) => {
    ...
    const payload = Object.fromEntries(new FormData(form).entries());
    const checkoutItems = getCheckoutItems();
    const totals = getCartDetails(checkoutItems);
    ...
    try {
      ...
      try {
        const response = await api("/api/checkout", {
          ...
          body: JSON.stringify({
            ...
            payment: {
              ...
              reference: payload.paymentNote || "",
              status:
                payload.method === "Online Pay (WhatsApp)" ? "Pending Confirmation" : "Pending",
            },
            items: checkoutItems,
          }),
        });
        ...
        const localOrder = createLocalOrder(
          {
            ...payload,
            cardNumber: payload.paymentNote || "",
          },
          checkoutItems
        );
      ...
      saveCart();
      form.reset();
      if (payload.method === "Online Pay (WhatsApp)") {
        openWhatsappPayment(payload, selectedItems, totals, createdOrderId);
        message.textContent = "Order placed. Opening WhatsApp for online payment.";
      } else {
        message.textContent = "Order confirmed. Cash on Delivery selected.";
      }
      updateCheckoutView();
      showOrderSuccessAndRedirect(createdOrderId);
    } catch (error) {
    ...
    return orders
      .map((order, index) => {
        ...
        return `<article class="admin-item">
          ...
          ${items}
          <div class="inline-actions" style="margin-top:8px;">
            <a class="small-btn" href="./invoice.html?id=${order.id}" target="_blank">📄 Tax Invoice</a>
          </div>
          ${actions}
        </article>`;
      })
...
function setupAdminForm() {
  ...
  form.addEventListener("submit", async (event) => {
    ...
    if (imageFile) {
      payload.image = await uploadFileThroughApi(imageFile);
    }
    const galleryFiles = Array.from(form.elements.galleryFiles?.files || []).slice(0, 4);
    if (galleryFiles.length) {
      const fileUrls = [];
      for (const file of galleryFiles) {
        fileUrls.push(await uploadFileThroughApi(file));
      }
